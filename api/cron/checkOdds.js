...
/**
 * Call the configured LLM to produce a short natural-language summary of
 * the current batch of alerts.  Returns a string or null on failure.
...
async function generateAIAlertSummary(alerts, teams) {
  ...
  const topTeamLines = teams.slice(0, 5).map((t, i) =>
    `${i + 1}. ${t.team}: ${t.odds.toFixed(1)}%`
  ).join('\n');

  const prompt = `You are a concise World Cup 2026 betting alert bot. Summarize these market movements in 2-3 sentences for a notification. Be direct and highlight the most significant change.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── Optional AI-enhanced alert messages ──
    if (alerts.length > 0 && process.env.AI_API_KEY) {
      try {
        const aiSummary = await generateAIAlertSummary(alerts, teams);
        if (aiSummary) {
          for (const alert of alerts) {
            alert.aiMessage = aiSummary;
          }
          console.log('checkOdds: AI-enhanced alert summary generated');
        }
      } catch (aiErr) {
        console.error('checkOdds: AI summary skipped:', aiErr.message);
        // Non-breaking: alerts still work with their original messages
      }
    }
